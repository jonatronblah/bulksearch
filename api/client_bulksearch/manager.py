...
svc_username = os.getenv("LDAP_USERNAME")
svc_password = os.getenv("LDAP_PASSWORD")
...
class BaseUserManager(Generic[models.UP, models.ID]):
    ...
    async def create(
        ...
        if ldap_auth:
            server = Server(LDAP_HOST, get_info=ALL_ATTRIBUTES)
            ldap_verify = ldap_login(server, user_create.username, user_create.password)
            if not ldap_verify:
                raise exceptions.InvalidID()
            ldap_group_member = ldap_group(server, svc_username, svc_password, user_create.username, RESTRICTED_GROUP)
            if not ldap_group_member:
                raise exceptions.InvalidID()
    ...
    async def authenticate(
        ...
        if LDAP_AUTH_FLAG:
            server = Server(LDAP_HOST, get_info=ALL_ATTRIBUTES)
            ldap_verify = ldap_login(server, user.username, credentials.password)
            if not ldap_verify:
                raise exceptions.InvalidID(user.username, user.email, credentials.password)
            ldap_group_member = ldap_group(server, svc_username, svc_password, 
                                        user.username, RESTRICTED_GROUP)
            if not ldap_group_member:
                raise exceptions.UserNotExists(user.username, user.email,credentials.password)
